...
void process3(int fd,uPcb* pt)
{
	...
	t.cmd=pt->position;
	strcpy(t.user,pt->username);
	t.len=pt->ppcode;
	send(fd,&t,26,0);
}
void process10(int fd)
{stmes mt;
	train t;int len;
	recv(fd,&mt,sizeof(stmes),0);
	char filename[15]={0};
	strcpy(filename,mt.filename);
	bzero(&t,sizeof(train));	
	recv(fd,&t,26,0);
	len=t.len;
	recv(fd,&t,26+len,0);
	...
	{ t.cmd=404;
		...
		bzero(temp,140);
		bzero(rete,1000);	
		sprintf(temp,"file(filename,filesize,user,filetype,MD5,procode,ppfile)values('%s',%ld,'%s','d','%s',%d,%d)",filename,filelen,mt.user,tempmd5,mt.procode,mt.ppfile);
		insert(temp);
	}else{
		t.cmd=405;
		send(fd,&t,4,0);
		bzero(temp,140);
		sprintf(temp,"file(filename,filesize,user,filetype,MD5,procode,ppfile,realcode)values('%s',%ld,'%s','d','%s',%d,%d,%d)",filename,filelen,mt.user,tempmd5,mt.procode,mt.ppfile,ppnum);
		insert(temp);
	}
}

void process11(int fd)
{ 
	stmes mt;
	train t;int len;
	recv(fd,&mt,sizeof(stmes),0);
	...
	char temp[140]={0};
	char rete[1000]={0};
	sprintf(temp,"select realcode from file where user='%s' and procode=%d and filename='%s'",mt.user,mt.procode,mt.filename);
	query(temp,rete);
	if(!strcmp(rete,"")||atoi(rete)==0)
	{ 
		bzero(temp,140);
		bzero(rete,1000);
		sprintf(temp,"select MD5 from file where user='%s' and procode=%d and filename='%s'",mt.user,mt.procode,mt.filename);
		query(temp,rete);
there:
...
{   train t;int len;
	...
	char bbb[20]={0};
	strcpy(bbb,t.user);
	sprintf(temp,"select salt from user where username='%s'",bbb);
	query(temp,rete);
	if(strcmp(rete,"")==0)
	{  
		t.cmd=101;
		...
		bzero(temp,200);
		bzero(rete,1000);
		sprintf(temp,"select passwd from user where username='%s'",bbb);
		query(temp,rete);
